...
def comb_c_r(ccc,f):
    g_cnt = 0
    f.write("1 буква все остольные не повторяются")
    # в полученно ccc элемент 1 и 2 повторяются 2 раза
    # 3 элемент повторяется kn раз
    #словарь без букв, которые сюда пришли и объязательны
    dd = list(set(d)-set(ccc))
...
def m_c(f):
    ...
    for i in d:
        dc=(i)
        gg_cnt += comb_c_r(dc,f)


    print("получено всего комбинаций каждая буква 2 раза ", gg_cnt)


    for ccc in combinations(['a','b','c','d','e','f'],2):
        print(ccc)

        gg_cnt1 += comb_c(ccc,f)
        if gg_cnt1 > 10**7:
            print("Может не хватить места на диске, превышен лимит комбинаций !!!")
            break
    print ("получено всего комбинаций 2 буквы 2 раза ",gg_cnt1)
